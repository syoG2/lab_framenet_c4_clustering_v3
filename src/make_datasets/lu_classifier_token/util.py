...
def preprocess_data(
    ...
    words.insert(data["target_word_idx"], tokenizer.convert_ids_to_tokens(1))
    words.insert(data["target_word_idx"] + 2, tokenizer.convert_ids_to_tokens(1))
    text = " ".join(words)
    inputs = tokenizer(
        text,
        return_tensors="pt",
...
def extract_entities(
    ...
    for prediction, data in zip(predictions, dataset, strict=True):
        ...
        for token, label_id in zip(all_tokens, prediction["pred_label_ids"], strict=True):
            # 特殊トークン以外をlistに追加する
            if token not in tokenizer.all_special_tokens and token != 1:
                tokens.append(token)
