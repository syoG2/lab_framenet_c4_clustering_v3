...
from lu_classifier_token01.util import extract_entities, id2label, label2id, preprocess_data, run_prediction
...
class Args(BaseModel):
    ...
    def model_post_init(self, __context):
        ...
        if self.output_exemplar_file == Path(""):
            self.output_exemplar_file = output_dir / Path(f"exemplars_{self.part_id}_token01.jsonl")
        if self.output_wordlist_file == Path(""):
            self.output_wordlist_file = output_dir / Path(f"word_list_{self.part_id}_token01.jsonl")
        if self.model_path == Path(""):
            self.model_path = Path(f"./src/make_datasets/lu_classifier_token01/models/{self.model_name}/best_model")
        if self.tokenizer_path == Path(""):
            self.tokenizer_path = Path(f"./src/make_datasets/lu_classifier_token01/models/{self.model_name}/tokenizer")
